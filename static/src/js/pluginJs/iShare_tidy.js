		 		...
		 		render: function(){
		 			...
		 			var	_containerHTML = '<div style="text-align: center;background-color: ' + _bgcolor + ';box-shadow: 1px 1px 4px #888888;padding: 8px 8px 4px;border-radius: 4px;' + _radius + '">',
		 				  ...
		 				  _imgHTML = '<div class="qrcode" style="width:' + this.settings.qrcodeW + 'px; height:' + this.settings.qrcodeH + 'px; overflow:hidden;"></div>',
		 					...
		 					_downArrowHTML = '</div><div style="' + _downFlag + 'position: relative;height: 0;width: 0;border-color: transparent;border-top-color: ' + _bgcolor + ';border-bottom: none;"></div>';
		      // 拼接WXHTML
		...
		return{
			...
			init: function() {
				...
				if(this.wx){
					...
					this.wx.init();
					// 加载qrcode库
					Util.loadjs('./src/js/pluginJs/qrcode.min.js', this.wx.startQR());
				}
			},
			bindEvent: function(){
			var me = this;
